...
class FakeGetdns(object):
    ...
    """
    def __init__(self, certificate_association_data):
        self._certificate_association_data = certificate_association_data
...
#                'replies_full': [<read-only buffer ptr 0x7fe2e0029e80, size 636 at 0x7fe2e4e58fb0>],
                'replies_tree': [{'answer': [{'class': 1,
                                              ...
                                              'rdata': {
                                                  'certificate_association_data': self._certificate_association_data,
                                                  # 'certificate_usage': 3,
                                                  # 'matching_type': 1,
                                                  # 'rdata_raw': "",
                                                  # 'selector': 1
...
class TLSATests(SynchronousTestCase):
    def test_tlsa(self):
        """
        L{dane.tlsa} returns a hex encoded signature.
        """
        expected = b'FOOBAR'
        self.assertEqual(
            binascii.hexlify(expected),
            dane.tlsa('example.com', 443, 'tcp', getdns=FakeGetdns(certificate_association_data=expected))
        )
